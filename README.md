# SGR tools

Herramientas locales (Flask) para contingente, calendario, conciliación y rendimientos del Fondo de Riesgo.

No es una web pública: corre en tu PC.

## Cómo usarla

1. Instalá [Python 3](https://www.python.org/downloads/) y tildá **Add python.exe to PATH**.
2. Entrá a esta carpeta y hacé doble clic en `run.bat`.
3. Dejá la ventana negra abierta.
4. En Chrome o Edge abrí: http://127.0.0.1:5055

La primera vez `run.bat` instala las librerías (`requirements.txt`) y hace falta internet.

## Qué no va en este repo

La carpeta `data/` (resultados procesados, flujos, A3500 guardado) no se publica. Cada computadora genera la suya al procesar los Excel.
